...
# 이만큼은 무조건 민다 — 탈출 조건을 아예 보지 않는다.
#
# **없으면 회피가 즉시 끝난다.** 회피를 시작하는 순간 그쪽 차선이 이미 보이고
# 있으면 탈출 조건이 첫 프레임부터 만족된다 (회전 기동에서 겪은 것과 같다).
AVOID_MIN_FRAMES = 90

# 피하는 쪽 차선이 이만큼 연속으로 **실제 피팅되면** 정상 조향으로 복귀한다.
# sliding_window 가 반대쪽을 외삽한 경우(fit_left/fit_right 가 None)는 제외된다.
AVOID_EXIT_FRAMES = 5

# 이만큼 밀었는데도 차선을 못 잡으면 **정지가 아니라 차선 추종으로** 복귀한다.
# 장애물은 이미 지나쳤을 가능성이 크고, 계속 밀고 있는 것이 더 위험하다.
AVOID_TIMEOUT_FRAMES = 150
