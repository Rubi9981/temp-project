...
#
# 회전 기동과 같은 방식으로 돈다 — 전방에 목표점을 **만들어** 넣어 얻은 고정
# 서보값을 그대로 내고, **차선 인식은 전혀 보지 않는다.** 예전에는 인식된
# 중심선을 옆으로 평행이동했는데, 회피 구간은 애초에 차선이 잘 안 잡히는
# 구간이라 밀 대상이 없으면 조향이 멈췄고 한쪽만 잡혀 외삽된 중심선이 나오면
# 그 가짜 중심선을 따라갔다.

# 클래스 -> 피하는 방향. 면적 임계는 위 DETECTION_AREA_ENTER 를 그대로 읽는다.
AVOID_SIDE = {'car_white': 'right', 'car_red': 'left'}

# 목표점을 전방 LOOKAHEAD_CM 지점에서 옆으로 미는 양 (BEV px). 회전의
# TURN_OFFSET_PX 와 같은 뜻이고, 크기만 작다. 나오는 조향:
#
...
# 되어 차선 변경이 아니라 급회전이 된다. 221 이상은 최대 조향에 포화해 값을
# 올려도 달라지지 않으므로, 유효 조정 폭은 60~200 이다.
AVOID_OFFSET_PX = 150
...
# 회피가 끝난 뒤 **반대쪽 서보로 꺾어** 차체를 다시 도로와 나란히 놓는
# 프레임 수. 0 이면 이 단계가 없다.
#
# AVOID_FRAMES 동안 한쪽으로만 꺾으면 옆으로는 원하는 만큼 나가지만 그 끝에서
# 차는 그 방향으로 **기울어진 채** 남는다. 곧바로 차선 추종으로 돌아가면 그
# 기울기를 차선 오차로 다시 잡느라 반대쪽으로 크게 넘어간다. 같은 크기의
# 반대 방향 꺾기를 짧게 넣어 각도만 되돌린다.
#
# 각도만 되돌리는 것이므로 AVOID_FRAMES 보다 **짧아야 한다** — 같은 길이면
# 옆으로 나간 거리까지 되돌아와 회피 자체가 사라진다.
AVOID_RECOVER_FRAMES = 30

# 회피가 끝난 뒤 이 프레임 수 동안 **같은 방향** 회피를 다시 걸지 않는다.
#
# 없으면 회피가 반복된다 — 다 피하고 나서도 그 차가 시야에 남아 면적 임계를
# 넘고 있으면 끝난 다음 프레임에 곧바로 다시 시작하고, 그때마다 중심선이 또
# 150px 밀려 차가 도로 밖으로 계단처럼 밀려난다. 회전의 TURN_COOLDOWN_FRAMES
# 와 같은 이유다.
#
# **방향별로 따로 센다.** 흰 차를 오른쪽으로 피한 직후 빨간 차가 보이면 왼쪽
# 회피는 즉시 걸려야 한다 — 트랙 4번이 그 두 대를 이어서 놓는 구간이고,
# 한쪽 쿨다운이 반대쪽까지 막으면 두 번째 차를 그대로 들이받는다.
#
# 세는 기준은 기동이 **끝난** 프레임이라 기동 길이(AVOID_FRAMES +
# AVOID_RECOVER_FRAMES)와는 무관하다. 피하고 지나온 차가 시야에서 벗어나
# 면적 임계 아래로 내려가는 데 걸리는 시간으로 잡는다.
AVOID_COOLDOWN_FRAMES = 300
