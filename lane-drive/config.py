...
# ==============================================================================
# NCNN 으로 내보낸 모델(폴더)을 쓴다. 원본 best_v3.pt 대비 맥 CPU 기준 2.19배
# 빠르면서 검출 수는 117 -> 119 로 사실상 동일하다 (obstacles 150장 측정).
# 되돌리려면 'best_v3.pt' 로 바꾸면 된다. 둘 다 .gitignore 에 걸려 추적되지
# 않으므로 Pi 로는 따로 복사해야 한다 (폴더라 scp -r).
YOLO_MODEL_PATH = os.path.join(os.path.dirname(BASE_DIR),
                               'object_detection', 'best_v3_ncnn_model')
YOLO_CONF = 0.25
