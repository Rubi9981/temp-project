...
- `car_*` 를 뺀 것은 규정상 정적 장애물이 **회피** 대상이지 정지 대상이 아니어서다.
  회피는 아래 별도 절이 맡는다 (`--no-avoid` 로 끈다).
...
트랙 4번(정적 장애물 / 2차선 변경) 구간. **흰 차는 오른쪽으로, 빨간 차는 왼쪽으로**
비껴간다. `--no-avoid` 로 끈다.

| 조건 | 동작 |
|---|---|
| `car_white` 면적 ≥ `DETECTION_AREA_ENTER['car_white']` | 중심선을 오른쪽으로 `AVOID_OFFSET_PX` 만큼 민다 |
| `car_red` 면적 ≥ `DETECTION_AREA_ENTER['car_red']` | 왼쪽으로 민다 |
| `AVOID_FRAMES` 경과 | 차선 추종으로 복귀 |

**고정 조향 원호가 아니라 차선 중심선을 옆으로 평행이동한다.**

```python
shifted.fit_center[-1] += sign * cfg.AVOID_OFFSET_PX
```

`detect.py` 가 한쪽 차선만 잡혔을 때 반대쪽을 세우는 것과 같은 기법이다. 다항식의
곡률이 그대로 남으므로 **차선을 계속 따라가면서 옆으로 비껴간다** — 고정 원호는
곡선 구간에서 도로를 벗어난다.

**복귀는 시간으로만 판정한다.** 예전에는 "피하는 쪽 차선이 N프레임 연속 잡히면"
이었는데, 회피 구간에서 차선 인식이 잘 안 돼 그 조건이 성립하지 않았다. 그래서
`AVOID_FRAMES` 가 **곧 회피 폭**이 된다 — `AVOID_SPEED` 와 곱해진 만큼 옆으로
이동하므로, 속도를 바꾸면 이 값도 함께 봐야 한다.

`AVOID_OFFSET_PX` 는 계산해서 정한 값이다:

| offset | 횡거리 | delta | 반경 |
|---|---|---|---|
| 100 | 4.4cm | 13.5° | 45.7cm |
| **150 (기본)** | **6.6cm** | **19.8°** | **30.5cm** |
| 200 | 8.7cm | 25.7° | 22.9cm |
| 221+ | — | 28.0° | 20.7cm (포화) |

**차선 폭 전체(457.5px)를 쓰면 안 된다** — 최대 조향으로 포화해 회전 기동과 같은
명령이 되고, 차선 변경이 아니라 급회전이 된다. 유효 폭은 60~200 이다.

#### 알아둘 것

**흰 차와 빨간 차는 매 프레임 독립적으로 판정된다.** 흰 차를 피해 오른쪽으로 가는
도중 빨간 차가 임계를 넘으면 그 자리에서 왼쪽으로 넘어가고 카운터가 다시 시작된다.
둘 다 임계를 넘으면 **면적이 큰 쪽**(더 가까운 쪽)을 피한다.

**장애물이 계속 보이면 끝나자마자 다시 시작한다.** 복귀에 쿨다운이 없어서, 90프레임이
지나 차선 추종으로 돌아간 다음 프레임에 다시 임계를 넘으면 또 회피에 들어간다.
실주행에서는 그 사이 장애물을 지나쳐 면적이 줄어들 것을 전제한 동작이다.

**차선을 잃으면 밀 대상이 없다.** `fit_center` 가 없으면 직전 조향을 유지한 채
`AVOID_FRAMES` 를 채운다 — 고정 원호로 빠지지 않는다.

**회피 속도는 감속 배율을 타지 않는다.** 교차로 직진·회전과 같은 규약이다.
`red` 나 `right_sign` 이 보인다고 회피 중 속도가 또 절반이 되면 기동이 두 배로
길어지기 때문이다.

**사람·화살표·빨간불이 회피보다 우선한다.** 판단 순서에서 3·4·5번이 앞이다.
