...
**묵은 탐지로는 새 행동을 시작하지 않는다.** 결과 나이가
`DETECTION_MAX_AGE_MS`(500) 를 넘으면 `_areas()` 가 `fresh=False` 로 알리고,
그때는 **회전을 시작하지 않고, 빨간불에 새로 서지도 않으며, 감속 배율을 갱신하지
않는다**(직전 값 유지). 이미 서 있었다면 **계속 선다** — 묵은 결과를 근거로
출발하는 쪽이 훨씬 위험하기 때문이다.

**"묵었다"를 "아무것도 안 보인다"로 뭉개지 않는 것이 요점이다.** 빈 목록으로
돌려주면 감속이 풀리고 사라짐 판정이 거짓 발동한다. 그래서 `_areas()` 는
`(areas, fresh)` 를 함께 돌려준다.

원격 추론의 정상 나이는 (`--yolo-every` 간격 + 왕복) ≈ 325ms 다. `YOLO_WATCHDOG_MS`
(2000)는 **아예 정지**시키는 임계라 그 사이 1.5초가 무방비였는데, 이 게이트가
그 구간을 막는다. 종료 요약에 `탐지 묵음 N (x%)` 가 찍히니 **비율이 높으면 링크를
의심**할 것. 로컬 추론(`--yolo`)은 `link` 가 없어 항상 신선으로 본다.
